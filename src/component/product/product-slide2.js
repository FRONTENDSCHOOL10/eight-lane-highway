...
(async function () {
  ...
  async function fetchProductsData() {
    if (!cachedProductsData) {
      cachedProductsData = await pb.collection("specialProducts").getFullList();
    }
    ...
    return async function () {
      ...
      async function handleClick(e, index) {
        ...
        try {
          ...
          await getSavedRecentProduct("specialProducts"); // 최신 데이터를 가져오기 위한 비동기 호출
        } catch (error) {
  ...
  getSavedRecentProduct("specialProducts");
  ...
  async function renderAddShoppingCart(id) {
    const item = await pb.collection("specialProducts").getOne(id);
    let changePrice =
      item.discount === 0
        ? item.price
  ...
  function CustomSwiper(className) {
    const swiper = new Swiper(`${className} .swiper-container`, {
      ...
      on: {
        init: function () {
          renderProductItem(className, this);
